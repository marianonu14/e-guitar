...
const CategoriesCard = (props) => {
    return ( 
        <div className="relative flex cursor-pointer rounded-2xl overflow-hidden shadow-xl">
            <Link href={`/categories/${props.id}`}>
                <div className="absolute z-20 grid items-center text-center w-full h-full lg:text-3xl text-xl text-white hover:text-slate-400">
                    <h1>{props.name}</h1>
